...
function MarqueeItem(props) {
  ...
  return (
    <Link href={uRL || "/brands"}>
      <div className={size === "Large" ? "w-80" : "w-56"}>
...
function MarqueeRow({ marqueeItem, width, size, repeat }) {
  return (
    <div
      className="flex bg-black"
      style={{ width: (width && repeat && width * repeat) + "rem" }}
    >
      {marqueeItem?.map((item, i) => <MarqueeItem key={i} size={size} {...lowercaseKeys(item)} />)}
    </div>
  )
}
